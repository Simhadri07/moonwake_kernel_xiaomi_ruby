...
static void ln8000_parse_cmdline(struct i2c_client *client)
{
	...
	dev_err(&client->dev,"product_name = %d, ruby = %d, rubypro = %d, rubyplus = %d\n", product_name, ruby ? 1 : 0, rubypro ? 1 : 0, rubyplus ? 1 : 0);
}
...
static int ln8000_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (product_name == RUBY) {
		...
		dev_err(&client->dev,"rubypro and rubyplus no need to probe ln8000\n");
		return -ENODEV;
	}
